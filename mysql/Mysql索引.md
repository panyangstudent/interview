...
2. Myisam使用的是非聚簇索引，非聚簇索引的两颗B+tree看上去没什么不同，节点的结构完全一致只是存储的内容不同而已，主键索引B+tree存储了主键，辅助键索引B+tree存储了辅助键。由于索引树是独立的，通过辅助键索引B+tree检索无需访问主键的索引树。innodb引擎使用的是聚簇索引，索引即文件本身。
