...
## mysql的innodb索引详解        
1. 当数据库一条记录里包含了多个字段时，一颗B+tree就只能存储主键，如果检索的是非主键字段，则主键所以失去作用，又变成顺序查找。这时因为在第二个要检索的字段上简历第二套索引，这个索引由单独的B+tree来组织，。有两种数据存储方式可以解决多个B+tree访问同一套表数据问题，一种叫聚簇索引，另一种种叫非聚簇索引。对于聚簇索引存储来说，行数据和主键B+tree存储在一起，辅助键B+tree只存储辅助键和主键，主键和非主键B+tree几乎是两种类型的树，对于非聚簇索引存储主键B+tree在叶子结点存储指向数据行的指针，而非主键。
